  ...
  def defines_has_many_finder_method(reflection)
    ...
    define_method(method_name) do
      if instance_variable_defined?(ivar_name)
        ...
        instance_variable_set(ivar_name, reflection.klass.find(:all, :from => "#{self.class.prefix}#{self.class.collection_name}/#{self.id}/#{method_name}#{self.class.format_extension}"))
      else
